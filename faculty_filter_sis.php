...
<?php
  ...
  function echo_grouped_array($this_grouped_array, $filter_name) {
    ...
    // case-sensitive
    global $FILTERNAME, $FALL, $SPRING, $SUMMER, $CURRENT_YEAR, $CURRENT_SEMESTER, $TITLE, $DISSERTATION, $COURSE, $ARTS, $SUBJECT, $YEAR, $SEMESTER, $INSTRUCTOR;
    ...
    if($filter_name == '') {
      ...
      // check for multiple entries (' ' delimiter)
    ...
    for($i = 0; $i< $faculty_count; $i++) {
      ...
      if($filtering) {
        if (!in_array($current_faculty, $filter_names)) {
          continue;
        }
      }
      ...
      for($j = 0; $j< $faculty_course_count; $j++) {
        ...
        */
        ...
        if($current_faculty_courses[$j][$SUBJECT] == $ARTS && $current_faculty_courses[$j][$COURSE] == $DISSERTATION) {
